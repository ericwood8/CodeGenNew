...
- `RequiresJunctionTable` — if true, the template is hidden/disabled unless the table is a many-to-many junction/bridge table (`TableSummary.IsJunctionTable` / `TableModel.IsJunctionTable`, §6). **Defaults to `false`**, unlike the two restrictions at the top: only a template built specifically for this shape (`SP_Junction`, `API_Junction`, `TS_JunctionComponent`, `WinUI3_JunctionEditor`) asks for it.
- `RequiresChildTables` — if true, the template is hidden/disabled unless at least one other table has a foreign key pointing back at this one (`TableSummary.HasChildForeignKeys` / `TableModel.HasAtLeastOneChildForeignKey`, §6). **Defaults to `false`**; only `WinUI3_DetailMasterScreen.tt` asks — a table with no such restriction is exactly what `WinUI3_DetailScreen.tt` is for instead.
- `RequiredPrimaryKeyShape` — one of `SingleColumn`, `SingleIntOrGuid` or `SingleInt` (`TableSummary.PrimaryKeyShape` / `TableModel.PrimaryKeyShape`, §6). **Absent by default** (no restriction beyond `RequiresPrimaryKey`). `RequiresPrimaryKey` alone only says a table has a key at all — several templates need a specific *shape* of key, because they build routes or repository calls that only work for that shape: `TS_Model.tt` (`SingleColumn`, since its interface just needs one property to be "the key" — any type, just not composite); `TS_Service.tt`/`TS_Component.tt`/`TS_DetailMasterComponent.tt` (`SingleIntOrGuid`, since their routes take the id as `{id:int}` or `{id:guid}`); `API_Crud.tt` and the WinUI3 CRUD-screen family (`SingleInt`, since `CS_Repo`'s `GenericRepo<T>` only ever takes a plain `int`). Before this existed, a table shaped wrong for one of these was only caught by the template's own `Error()` call once generation was actually attempted — the menu (and the CLI) would still *offer* an option that could never work for that table. Discovered by generating live against real, messy legacy schemas (composite keys and natural `varchar`/`char` keys are common there) rather than only hand-built fixtures. `TableModel`/`TableSummary`'s own `PrimaryKeyShape` is unrestricted (`None`/`Composite`/`SingleInt`/`SingleUniqueIdentifier`/`SingleOther`) since it describes what a table actually has, not what a template needs — `RequiredPrimaryKeyShape`'s three values are the tiers actually needed across the shipped templates, not a 1:1 mirror. The junction/child-grid families (`SP_Junction.tt`, `WinUI3_JunctionEditor.tt`, `API_Junction.tt`, `TS_JunctionComponent.tt`) set nothing here: their data access goes by the two association columns or the child's own foreign key, never by the table's own primary key shape (a junction table's PK can legitimately be composite).
- `RequiresNotNameActiveTable` — if true, the template is hidden/disabled for a "name/active" table: a NOT NULL text column called exactly `Name` plus a NOT NULL bit column called exactly `IsActive` (`TableSummary.IsNameActiveTable` / `TableModel.IsNameActiveTable`, §6 — the one exact-name shape test that used to be spelled out inline, identically, in `API_Crud.tt`, `CS_Entity.tt`, `CS_Repo.tt` and the WinUI3 CRUD-screen family; now canonical on `TableModel` so they can't drift apart). **Defaults to `false`**. Such a table's repository is a `NameActiveRepo`, not a `GenericRepo` — it needs duplicate-name checks and trimming its API can't get from a template, so `API_Crud.tt` has always refused it (`CS_Entity.tt`/`CS_Repo.tt` do NOT set this: they deliberately support both shapes, branching to the right base class). Set on every template whose generated code assumes a plain `getAll()`/`GetById`/`Create`/`Update`/`Delete`-shaped backend: `TS_Service.tt`, `TS_Component.tt`, `TS_DetailMasterComponent.tt`, `API_Crud.tt`, and the WinUI3 CRUD-screen family (`WinUI3_MasterScreen.tt`, `WinUI3_DetailScreen.tt`, `WinUI3_DetailMasterScreen.tt`). Found live: the real `TimeEntry` database's `DepartmentTeamApi.cs` (a hand-maintained name/active API) has no plain `GET api/departmentteams` route at all — only `GET api/departmentteams/department/{id}` (scoped to one parent) and `GET api/departmentteams/{name}` — so `TS_Service.tt`'s always-generated `getAll()` would have called a route that 404s. `TS_Model.tt` is unaffected (it's just the JSON interface shape, no API call at all); the junction/child-grid templates are unaffected (their data access goes through their own dedicated endpoints, never a plain `getAll()`).
...
## 6. Database Introspection: `TableModel` / `ColumnModel` / `ForeignKeyModel` / `ChildForeignKeyModel`
...
public class TableModel
{
    ...
    // None / Composite / SingleInt / SingleUniqueIdentifier / SingleOther -- see section 5.3's
    // RequiredPrimaryKeyShape for why "has a primary key" alone isn't enough for several templates.
    public PrimaryKeyShape PrimaryKeyShape { get; }

    // A NOT NULL "Name" text column plus a NOT NULL "IsActive" bit column -- see section 5.3's
    // RequiresNotNameActiveTable. The one canonical home for a shape test several templates used to spell out inline.
    public bool IsNameActiveTable { get; }
    ...
    // A many-to-many junction/bridge table: excluding computed columns, audit-classified columns
    // (IsAuditColumn), and a surrogate identity primary key column, exactly two columns remain, each
    // individually covered by its own single-column FK (same or two different parent tables -- a
    // self-referencing many-to-many edge counts too). Recognizes BOTH real-world shapes: a natural
    // composite key (the two FK columns ARE the primary key) and a surrogate-id key (an identity PK plus
    // two plain FK columns, e.g. a real dbo.NameBaseGroupXref table on a second production database used
    // for testing -- confirmed against a live database, 2026-09-25, to be the more common shape, not the
    // composite-key one first assumed). Purely
    // structural, derived from data already above -- needs no extra schema query. A composite FK spanning
    // both columns in one constraint does NOT count (that's a key duplicating a parent's own composite
    // key, not an association).
    ...
    public bool IsJunctionTable =>
        ...
        JunctionCandidateColumns.All(c => ForeignKeys.Any(fk => fk.ReferencingColumns.Count == 1 && fk.ReferencingColumns[0] == c.Name));

    // The two FKs making up the association (in JunctionCandidateColumns order), for a template to read
    // ReferencedTable/ReferencedDisplayColumns/etc. from without re-deriving which FKs they are.
    public List<ForeignKeyModel> JunctionForeignKeys { get; }

    // The mirror image of ForeignKeys: other tables whose FK points back at this one. Always computed,
    // same as ForeignKeys (not gated behind a .tt.config flag) -- lets a template discover, e.g., that
    // E_TimeSheetDetail depends on E_TimeSheet, without a person having to say so.
    public List<ChildForeignKeyModel> ChildForeignKeys { get; init; }
...
public class ColumnModel
{
    ...
    public bool IsFilePathColumn { get; init; }         // category "FilePathColumn" (e.g. LogoFile, PhotoPath); not yet consumed by any shipped template
...
// The mirror image of ForeignKeyModel: describes another table's FK pointing back at THIS table (found via
// sys.foreign_keys/sys.foreign_key_columns filtered by referenced_object_id instead of parent_object_id --
// the same join as ForeignKeyModel's own query, just read from the other side). Added to let a template
// discover a table's "children" (e.g. an invoice's line items) without a person having to pair them up by
// hand -- unlike ForeignKeyModel, the referencing table has to be named here since it's no longer "self".
public class ChildForeignKeyModel
{
    public string ConstraintName { get; init; }
    public string ReferencingSchema { get; init; }
    public string ReferencingTable { get; init; }
    public List<string> ReferencingColumns { get; init; }
    public List<string> ReferencedColumns { get; init; }   // the column(s) on THIS table the child points at (usually the PK)
}
```
...
**A sixth behavior**, `ModifiedUserColumn` (e.g. `ModifiedBy`, `UpdatedBy`), was added after a survey of ~35 real databases on the author's own SQL Server (2026-09-22) turned up "who last touched this row" as a common companion to `ModifiedDateColumn`. It is the Update-side mirror of `CreateUserColumn`: a normal caller-supplied parameter on `SP_Update.tt` (no special handling needed there — it just isn't excluded), but left out of `SP_Insert.tt`, `SP_Save.tt`'s insert branch, `SP_Load.tt` and `SP_Clone.tt` entirely, since a brand-new, seeded or freshly-cloned row has no prior editor. Its patterns (`ModifiedBy*`, `UpdatedBy*`) were deliberately chosen so they cannot also match `ModifiedDateColumn`'s own patterns — the config format has no shared-priority mechanism between categories, so avoiding overlap in the pattern lists themselves is what keeps the two from ever claiming the same column (`SpecialLogicColumnsConfigTests.ModifiedUserColumn_and_ModifiedDateColumn_never_both_claim_the_same_column` guards this).

That same survey found `IsActiveFlag`, `StartEndDate` and `SoftDelete` had shipped **case-sensitive** (unlike every other category), silently missing the ALL-CAPS/snake_case naming common in older schemas (`BEGIN_DATE`, `IsInActive`, `DELETED_ON`, ...). All three are now `IgnoreCase`, and a handful of exact snake_case spellings (`Is_Active`, `Is_Deleted`, ...) were added directly since the wildcard engine has no "ignore underscores" mode. `CreateDateColumn`, `ModifiedDateColumn` and `DisplayColumn`'s pattern lists were similarly broadened with real spellings from the survey (`Date_Created`, `Modified_On`, `*Description`, `Label`, ...) — deliberately *not* with broad catch-alls like `*Updated` or `*Created`, which would also match a same-shaped non-date column (a `RecordsUpdated` count, an `IsUpdated` flag) and generate a `SET column = GETDATE()` into it. Every addition is a name spelling actually observed across multiple of the surveyed databases, per the project's own "get the common ones, not every one-off" guidance — see the `SpecialLogicColumns.config` comments for the reasoning.
...
Most templates produce one file, named by `TemplateInfo.BuildFileName` (or by the `OutputName` key of §5.3). A template that must write **several** files — or a file inside sub-folders, like an Angular component's four files under `components\<name>\` — says so itself by writing a marker line before each file:
...
- Children = tables (views excluded until v2), from `SqlServerSchemaProvider.ListTablesAsync()` — a lightweight, read-only query (table name, whether it has a PK, whether it has any unique index, its `PrimaryKeyShape`, plus the bulk-computed `IsJunctionTable`/`HasChildForeignKeys`/`IsNameActiveTable` equivalents of §6's `TableModel` properties, each read via its own catalog-only query across every table at once rather than a per-table round trip) kept deliberately cheap since it runs for every table up front, unlike the full `TableModel` build which only happens for the one table actually selected for generation.
...
- **Right-click menu** (`RightTapped` on each row) is built fresh every time from `MainViewModel.GetApplicableTemplates(table)` — `TemplateCatalog.Discover(...)` filtered by `TemplateInfo.AppliesTo(hasPrimaryKey, isView: false, isJunctionTable, hasChildForeignKeys, primaryKeyShape, isNameActiveTable)` — grouped into `MenuFlyoutSubItem`s by `SubmenuGroup` exactly as §8 describes, so adding a new `.tt` file changes the menu with no code change, GUI included. A template whose `.tt.config` sets `RequiredPrimaryKeyShape` (section 5.3) is left off a table's menu entirely when the key is the wrong shape for it (composite, or a natural `varchar`/`char` key for a template that needs `int`/`uniqueidentifier`) — found by generating live against real legacy schemas where that shape is common, rather than only right-clicking and finding out. Likewise `RequiresNotNameActiveTable` (section 5.3) leaves a "name/active" table (`Name` + `IsActive`) off the menu for every template that assumes a plain `getAll()`-shaped backend — found the same way, against the real `TimeEntry` database's own hand-maintained `DepartmentTeamApi.cs`, whose only "list" route is scoped to a parent rather than a bare `getAll()`.
...
- Twenty-eight shipped templates (nine stored procedure templates, two API templates, four C# templates, five Angular TypeScript templates, four React TypeScript templates and four WinUI3 templates; all in `Templates\`; on disk they are `SP_Insert_v1.tt` etc. — see §8.0 — while this document uses the unversioned names), each tested against a real database using the manual protocol above:
  ...
  - **`CS_Validation.tt`** — a validation "buddy class" (`CodeGenPossibilities\ValidationLibrary` in the research folder): a `<Table>Metadata` class carrying `[Required]`/`[StringLength]`/`[DataType]`/`[Display]` attributes, attached to the real entity via `[MetadataType(typeof(<Table>Metadata))]` on a re-opened `partial class <Table>` rather than written onto the entity directly — so it works for a hand-maintained entity too, not just a `CS_Entity.tt`-generated one. `CS_Entity.tt` already writes `[StringLength]`/`[DataType]`/`[Display]` itself (its own header comment lists `[Required]` validation as something it deliberately does not write); `[Required]` is this template's actual new contribution — C#'s `required` modifier (which `CS_Entity.tt` does use) is a compile-time obligation, not the same thing as a runtime `[Required]` check. Validates every column except computed columns, audit-classified columns (`IsAuditColumn` — server-set, not something a caller fills in a form), and an **identity** primary key; a non-identity (natural or caller-supplied) key is still validated, the same reasoning `SP_Insert.tt` uses to decide whether the key is a parameter. `[DataType]` comes from a semantic name-pattern match — `*Email*`/`*Password*` are new here, `*Phone*` and the 100+-character `MultilineText` threshold match `CS_Entity.tt`'s own rule, checked in that order so a semantic name wins over a generic length guess. Deliberately does not write EF-mapping or display-formatting attributes (`[Column]`, `[ForeignKey]`, `[Precision]`, `[DatabaseGenerated]`, `[Timestamp]`, `[DisplayFormat]`) — those belong to the entity's own mapping and would be redundant or conflicting from a second class — nor `[Range]`/`[RegularExpression]` (needs judgment `CodeGenNew` cannot supply) or the research example's custom `[RequiredIntForeignKeyValidator]` attribute (assumes a hand-written attribute class no target project can be assumed to have; a `NOT NULL` foreign key still gets a plain `[Required]`). Unlike `CS_Entity.tt`, does **not** require a single-column key — a validation-attribute carrier has no `[Key]`/Fluent-API concern, so a composite-key table's columns can still be validated even though `CS_Entity.tt` itself would refuse to generate an entity for it. One integration step is left to the developer, the same category as `TS_Component.tt`'s own route/sidebar/`app.config` checklist: the target entity class must be declared `partial` for `[MetadataType]` to attach (`CS_Entity.tt`'s own output is not `partial` today), and whatever actually calls `Validator.TryValidateObject` (or wires ASP.NET Core's own model-validation pipeline) is up to the target project. Generated live against a second production database's `dbo.Products` table: the identity `ID` column correctly excluded, the three `*ProductTypeID` foreign keys and the two boolean flags correctly `[Required]`, `Description`/`GEProductCode` correctly `[StringLength]`, and the nullable `GEProductCode` correctly left without `[Required]`.
  ...
  - **`CS_Repo.tt`** — the thin per-table repository class `<Table>Repo.cs` (`OutputName={Table}Repo.cs`) in the style of TimeEntryServer's `TimeEntry.Common\Repositories\HolidayRepo.cs`: a constructor over `TimeEntryContext` on top of `NameActiveRepo<Entity>` (a table with a NOT NULL text `Name` and NOT NULL bit `IsActive`, the same test `CS_Entity` uses) or `GenericRepo<Entity>`. The shared `GenericRepo`/`NameActiveRepo` and their interfaces are hand-written once and not generated. Extra queries are written only where the columns alone decide them: `GetAllOf<Parent>(int id)` (active rows for the parent, by name) on a name/active table with exactly one NOT NULL int foreign key to an entity table, `GetByName(string name)` on a non-name/active table with a NOT NULL text `Name`, and — added 2026-09-27, `CodeGenPossibilities\AdvancedRepositoryPatterns` in the research folder — `HasDuplicate<Column>(value, excludeId)` for every column that takes part in a UNIQUE index or constraint other than the primary key (`ColumnModel.IsInUniqueIndex`, the same signal `SP_Clone.tt` already uses for its override parameters): does some other row already have this value, the generalized shape of a real `CityRepository.HasDuplicates` method found in the research sweep. One `HasDuplicate` per column, not one per composite unique constraint — `IsInUniqueIndex` only says a column takes part in *some* unique index, not which other columns share a composite one. Queries that `Include` navigation properties or use a hand-picked order stay hand-maintained. **No repository is written for the enum tables, the other `*Type` lookup tables or tables with no entity class** — the template's `noRepositoryTables` list makes them stop with an error. Verified on the `TimeEntry` database: `DepartmentTeamRepo`, `ProjectTaskRepo` and `HolidayRepo` regenerate identically apart from one comment; six repositories that carry `Include` queries or an unusual base (`Department`, `Employee`, `Project`, `E_RequestExpenseSheet`, `E_TimeSheetDetail`, `TimeEntryUser`) were left hand-maintained; six tables with no repository yet got a plain one, and the solution compiled. `HasDuplicate<Column>` verified live against the second production database's `dbo.Account` table: correctly generated `HasDuplicateAccountNumber(string accountNumber, int excludeId)` for its one non-PK unique-indexed column (`AccountNumber`), comparing against the table's real primary key column (`ID`). Also added, 2026-09-27, `CodeGenPossibilities\SuggestUniqueName` in the research folder — `SuggestUnique<Column>(string desired)` for every column `HasDuplicate<Column>` would cover that is *also* a string column (appending a numeric suffix to a non-text unique column makes no sense): returns `desired` unchanged if free, else the same value with a bumped numeric suffix until one is found that no row already has, the generalized shape of a real `EnsureNameIsUnique`/`IsUniqueName` helper pair found in a second, later research sweep that needed a hardcoded per-entity-type switch statement in its own shared base class — generating one method per table needs no switch at all. Checks table-wide, the same scope (and the same composite-unique-constraint caveat) `HasDuplicate<Column>` already uses. Verified live against the same second production database's `dbo.Account` table: correctly produced `SuggestUniqueAccountNumber(string desired)` alongside `HasDuplicateAccountNumber`.
  - **`TS_Model.tt`**, **`TS_Service.tt`**, **`TS_Component.tt`** — the Angular TypeScript layer of `TimeEntryUI` (`src\app\models`, `services`, `components`), written with the multi-file marker of §8.2 so each lands in its own folder (`models/holiday.ts`, `services/holiday.service.ts`, `components/holiday/holiday.component.{css,html,spec.ts,ts}`). All three name things from the table without its `E_`/`SY_` prefix (`E_TimeSheet` → `TimeSheet`, files `timesheet…`), use the same service URL the API registers (`api/<lower name>s`), and refuse tables with no API (enum/`*Type`/entity-less tables) or (for `TS_Service`/`TS_Component`) without a single `int` or `uniqueidentifier` key — a GUID key is a `string` in the model and service, the route is expected to take `{id:guid}`, and a new row is sent with the empty GUID so the API assigns the key (`TS_Model` accepts any single-column key). `TS_Component` loads all rows with no paging, and shows a table's long-text columns in the grid only when it has no other columns. **`TS_Service`/`TS_Component` also refuse a "name/active" table** (`RequiresNotNameActiveTable`, section 5.3): its real API is always hand-maintained and — confirmed against the real `TimeEntry` database's `DepartmentTeamApi.cs` — commonly has no plain `getAll()` route at all, only a parent-scoped one (`/department/{id}`) and a by-name one; `TS_Service`'s always-generated `getAll()` would have called a route that 404s.
  ...
  - **`TS_DetailMasterComponent.tt`** (`RequiresChildTables=true`, `RequiresNotNameActiveTable=true`, `NeedsReferencedDisplayColumns=true`) — `TS_Component.tt`'s own grid and form (identical field selection, lookups and rules) plus one read-only grid per table in `TableModel.ChildForeignKeys` (§6), shown under the form while editing an existing row; only offered when there's at least one such table (a table with none is exactly `TS_Component.tt`'s own job). The Angular counterpart of `WinUI3_DetailMasterScreen.tt`, closing the gap `TS_Component.tt`'s own header comment names explicitly ("not written: a detail grid — Department's teams, Project's tasks, TimeSheet's lines"). Per the same answer that shaped `WinUI3_DetailMasterScreen.tt`, every child table gets a grid, unconditionally. A child grid calls the child table's API **directly** (`this.http.get<any[]>('api/<child>s')`, filtering client-side by the foreign key column) instead of importing a generated `<Child>Service`/`<Child>` model the way every other lookup in this template does — `TableModel` is built for exactly one table per generation run, so the template never knows the child's own column list or its service's real shape, and `TS_Service.tt`'s own header comment already names a real example (`TimeSheetDetailService`) whose hand-written service has neither a `getAll()` method nor the standard method names. Rows are loosely typed (`any[]`); the columns shown are whatever keys the first loaded row actually has (`Object.keys`), discovered at **run time** rather than generation time — the Angular-side version of the same problem `WinUI3_DetailMasterScreen.tt` solves with EF Core metadata. A child grid only loads once the row being edited has a real key (a brand-new, unsaved row has none yet). Generated live against the real `E_TimeSheet`/`E_TimeSheetDetail` pair (the `TimeEntry` sample database) and dropped into the real `TimeEntryUI` project, where `ng test` compiled and ran it for real (12/12 passing) — but only after temporarily setting aside the hand-maintained `components/timesheet/` and swapping in a freshly generated `timesheet.ts`: the real, hand-written model types its date column as `Date`, not the `string` every `TS_*` template assumes, a pre-existing mismatch this exercise surfaced (not something this template caused) — both were restored afterward via `git checkout`, confirmed clean.
  - **`TSX_Api.tt`**, **`TSX_Page.tt`**, **`TSX_DetailMasterPage.tt`**, **`TSX_JunctionPage.tt`** — the React counterpart of the whole `TS_*` family, written against a real React project (`CriticalViewer`, a small movie-review app) the same way the `TS_*` family was written against `TimeEntryUI`. No separate React model template exists: `TS_Model.tt`'s own output is a plain TypeScript interface with no Angular dependency, so it is reused as-is for a React project. `TSX_Api.tt` is `TS_Service.tt`'s shape as a plain object of `fetch`-based functions instead of an `@Injectable` class over `HttpClient` (React has no DI container), assuming the target project already supplies a shared `request<T>(path, init)`/`ApiError` pair (`import { request } from './client'`) the same way `TS_Service.tt` assumes Angular's own `HttpClient`. `TSX_Page.tt`/`TSX_DetailMasterPage.tt`/`TSX_JunctionPage.tt` mirror `TS_Component.tt`/`TS_DetailMasterComponent.tt`/`TS_JunctionComponent.tt`'s field selection, lookup-dropdown and child-grid rules exactly, as one function component per table (`useState`/`useEffect`, controlled inputs) with a colocated `__tests__` test file instead of Angular's four-file-per-component/`.spec.ts` split. Two departures from the Angular family found by inspecting the real target project rather than assumed up front: errors are `error` state rendered as `{error && <p>{error}</p>}`, never `alert()` (CriticalViewer's own pages never call `alert()` anywhere); and no CSS class is invented beyond `.rail` (CriticalViewer defines no `btn`/`form-group`/`form-container`/`btn-action` classes the Angular family assumes TimeEntryUI already has — carrying that assumption over uncritically would have generated wrongly-classed elements). Verified with 19 new tests (full suite 226/226) and a live CLI smoke test against the real `CriticalViewer` database's `Movies` table, which surfaced two further, pre-existing findings shared with the Angular family rather than introduced by the new templates: its inherited "≥100 characters is a textarea, excluded from the grid" rule leaves a generated grid nearly empty when a table's `varchar` columns are all generously sized for headroom rather than because the data is long (every column on `Movies` except `ReleaseYear` is ≥100 characters); and a table whose real name is already plural (`Movies`, not `Movie`) broke the family's "lower-case name + s" pluralization (`/api/moviess`), an edge case no singular-named sample table in this repo's own test suite had exercised. **Fixed** in both families, then centralized: `CodeGenNew.Core.Pluralizer.Pluralize()` — a shared extension method every template calls (`stem.Pluralize()`) instead of each copying its own logic — special-cases the endings regular English pluralization always produces from a *singular* word (`ss`/`x`/`z`/`ch`/`sh` → append "es" — a real production `Address`/`ARClass` table needs `Addresses`/`ARClasses`, not to be left alone) and otherwise treats a bare trailing "s" as already-plural (`Movies`, `SettingsSales`). The same audit found the identical bug, never previously exercised, in `TS_Component.tt`/`TSX_Page.tt`'s own list-holding variable name (`holidays`) — fixed the same way. A bare-"s" singular like `Bus`/`Status`, and true irregular/foreign plurals (`child`/`children`, `cactus`/`cacti`), are still out of scope by design, not oversight — a real inflector library was considered and declined as disproportionate to how rarely a bare singular table name is irregular English; those rare cases need the generated name corrected by hand. A fuller gap analysis comparing generated output against `CriticalViewer`'s actual hand-written screens — including a real, concrete finding that the family's schema-driven approach has no concept of a "sensitive column" and would generate a plain visible text box for a password-hash column — lives in `CodeGenPossibilities\ReactTemplateFamily` in the research folder.
  ...
  - **`SP_Search.tt`** — a list/search screen's dynamic filter (`CodeGenPossibilities\SearchFilterBuilder` in the research folder): one `@Column` parameter per searchable column, defaulting to `NULL` (not filtered on); every supplied parameter contributes an AND-ed `LIKE '%value%'` clause, trimmed first (the same "trim strings" rule `SP_Insert.tt` follows), so a caller can filter by any combination of the columns it exposes, or none. Searchable columns are every string column that isn't audit-classified (`IsAuditColumn`) — nobody types into a search box for "who created this row" — and the template refuses outright when a table has none (e.g. all-numeric/date/bit columns, or a junction table whose only string column is `CreateUser`). Returns every column, not just the display columns `SP_Lookup.tt` returns, since a search results grid usually wants more than just the row's identity; `ORDER BY` reuses `SP_Lookup.tt`'s "best display column, then primary key" convention. Deliberately does not filter out inactive/soft-deleted rows — an orthogonal concern left to the caller. No index is assumed for any searched column (a leading-wildcard `LIKE` is an O(n) scan per filtered column), documented as a limit in the generated file's header rather than silently generated as if free. Generated live against a second production database's `dbo.Products` table (two searchable columns found, `Description`/`GEProductCode`, correctly excluding the `ID`/type-FK/bit columns) and against `NameBaseGroupXref` (correctly refused: its only string column, `CreateUser`, is audit-classified).
  ...
  - **`SP_Junction.tt`** (`RequiresJunctionTable=true`, `NeedsReferencedDisplayColumns=true`) — the first template restricted to `TableModel.IsJunctionTable` (§7): three procedures, `<Table>_List`/`_Link`/`_Unlink`, for editing one side of a many-to-many association. Deliberately does **not** reuse `SP_Insert`/`SP_Delete`, because a junction table's association columns are only sometimes its primary key — `IsJunctionTable` also recognizes a surrogate-identity-key shape where they aren't, so Link/Unlink work by the two association columns directly rather than by whatever the table's real primary key happens to be. `JunctionForeignKeys[0]` is treated as the "anchor" (the row already picked elsewhere) and `[1]` as the "target" (the side being multi-selected) — a naming convention, not something the schema states; swap by hand if a table's natural direction reads the other way. `_List` CASTs each display column to `NVARCHAR(500)` regardless of its real type (the same "a display column is something to show, not typed data" convention `SP_Update.tt` already established for its own parameters) so the WinUI3 side never needs to know a display column's real SQL type. `_Link` is idempotent (`IF NOT EXISTS`) and honors `CreateDateColumn`/`CreateUserColumn` when present. Generated from and tested against a real many-to-many table, a second production database's `dbo.NameBaseGroupXref` (surrogate `ID` primary key; `NameBaseID`/`GroupID` plain FK columns — real data confirmed this is the *more* common junction-table shape, not the composite-key one first assumed), via a disposable `BACKUP ... WITH COPY_ONLY`/`RESTORE` scratch copy: `_List` correctly flagged the two already-linked `Groups` rows, `_Link` added a third and a repeat call did not duplicate it, `_Unlink` removed one of the original two, and a final `_List` reflected all three changes — the real database was confirmed unchanged (still 6 rows) after the scratch copy was dropped.
- **`API_Junction.tt`** (`RequiresJunctionTable=true`, `NeedsReferencedDisplayColumns=true`) — the HTTP companion to `SP_Junction.tt`, needed because a browser-based screen can't reach a `DbContext` directly the way `WinUI3_JunctionEditor.tt`'s ViewModel does: `GET /<plural>/junction/{anchorId}`, `POST /<plural>/junction/link`, `POST /<plural>/junction/unlink`, registered from `Register()` the same way `API_Crud.tt` does (`BreakIntoStrings`). Unlike `API_Crud.tt`, there is no `<Table>Repo` to go through — the handlers call `SP_Junction.tt`'s three procedures straight through the injected `DbContext` (`SqlQueryRaw<T>`/`ExecuteSqlInterpolatedAsync`), the same call shape `WinUI3_JunctionEditor.tt`'s ViewModel uses, so the two consuming templates' data-access conventions can't drift apart. Generated against the real `NameBaseGroupXref` table, then dropped into the real `TimeEntryServer` project (`TimeEntry.ApiService\Apis\`) and built there — 0 errors, using the project's real `BaseApi<T>`/`TimeEntryContext` — before being removed again (never committed).
- **`TS_JunctionComponent.tt`** (`RequiresJunctionTable=true`, `NeedsReferencedDisplayColumns=true`) — the Angular counterpart to `WinUI3_JunctionEditor.tt`: a standalone component (`components/<table>-junction/`) with two `<select multiple>` lists and `<<`/`<`/`>`/`>>` buttons, calling `API_Junction.tt`'s three endpoints directly via `HttpClient` — no separate `TS_Service`-shaped class, since a junction table has only List/Link/Unlink, folded straight into the component the same way `TS_Component.tt`'s own "plain component shape" already combines grid and form. `anchorId` is an `@Input()`; this component is meant to be embedded in whatever screen already has the anchor row selected, not routed to on its own. Generated against the real `NameBaseGroupXref` table, then dropped into the real `TimeEntryUI` project and verified with `ng test --watch=false --browsers=ChromeHeadless` (the same headless-Chrome method `TS_Component.tt` was verified with, above) — 13/13 passing (the pre-existing 12 plus this one) — before being removed again (never committed).
- **`WinUI3_JunctionEditor.tt`** (`RequiresJunctionTable=true`, `NeedsReferencedDisplayColumns=true`) — the first template outside the SP/API/CS/TS families and the first to target a desktop UI framework: a two-list "available/selected" shuttle-control `ContentDialog` (`Views/<Table>JunctionEditor.xaml`/`.xaml.cs`, `ViewModels/<Table>JunctionEditorViewModel.cs`) for editing a junction table's associations, the UI companion to `SP_Junction.tt` (calls its three procedures via the target project's own EF Core `DbContext` — the same `contextType` setting `CS_Repo.tt`/`API_Crud.tt` already use — through `Database.SqlQueryRaw<T>`/`ExecuteSqlInterpolatedAsync`). Each Add/Remove click commits immediately (one `_Link`/`_Unlink` call) rather than batching for a single Save button. Assumes nothing from `CodeGenNew.App` itself — a plain `ContentDialog`, no shared house-style helper — the same trust model `CS_Repo.tt` already applies to `GenericRepo`. Generated against the same real `NameBaseGroupXref` table and hand-reviewed, but **not** compiled in a live WinUI3 project: unlike `TS_JunctionComponent.tt`/`TS_Component.tt` (verified against the real `TimeEntryUI` Angular app), there is no existing WinUI3 desktop client in this ecosystem to drop the output into.
- **`WinUI3_MasterScreen.tt`**, **`WinUI3_DetailScreen.tt`**, **`WinUI3_DetailMasterScreen.tt`** — the desktop CRUD-screen family (`CodeGenPossibilities\DesktopCrudScreens`/`MasterDetailChildGrid` in the research folder), together with `CS_Repo.tt` covering the same "list + add/edit a row" shape `TS_Component.tt` covers for Angular, but as two (or three) separate WinUI3 screens instead of one combined component — a desktop app's window model suits a separate list `Page` and edit `ContentDialog` better than a browser's single-page-with-inline-form convention. All three share `WinUI3_DetailScreen.tt`'s field selection (every column except the primary key, computed and identity columns; a foreign key becomes a `ComboBox`, a bit column a `CheckBox`) and require a single, non-composite **`int`** primary key while refusing a "name/active" table (`Name` + `IsActive`) — the same restriction `API_Crud.tt` uses, for the same reason: their `<Table>Repo` calls (`GenericRepo<T>`) assume a plain `int` key, and a name/active table's `NameActiveRepo` has no `GetAll` for the master screen to call.
  - **`WinUI3_MasterScreen.tt`** (`NeedsReferencedDisplayColumns=true`) — `Views/<Table>ListPage.xaml`/`.xaml.cs`, `ViewModels/<Table>ListViewModel.cs`: a `Page` (not a WinUI3 `DataGrid`, which needs a third-party package no target project can be assumed to have) listing every row of `<Table>Repo.GetAll()` via a generic `<Table>ListRow.Cells` (`List<string>`, one cell per `WinUI3_DetailScreen.tt` field) so the XAML needn't hand-declare one column per database column. A foreign key column resolves to the parent's display column (one dictionary lookup per distinct parent, loaded once) instead of showing the raw id. Add/Edit open `WinUI3_DetailScreen.tt`'s dialog; Delete confirms first, then maps `GenericRepo.DeleteAsync`'s 0/-1/other results to reload/"row no longer exists"/"row is in use".
  - **`WinUI3_DetailScreen.tt`** (`NeedsReferencedDisplayColumns=true`) — `Views/<Table>DetailDialog.xaml`/`.xaml.cs`, `ViewModels/<Table>DetailViewModel.cs`: a `ContentDialog` add/edit form. WinUI3's compiled `x:Bind` cannot convert a `TextBox.Text` string into a numeric/date/`Guid` property without a converter class no target project can be assumed to have, so every such field is a **string-backed** property on the ViewModel (parsed into the entity's real type, with a validation message, inside `SaveAsync`) rather than bound to the typed entity property directly; a `ComboBox.SelectedValue`/`CheckBox.IsChecked` bind straight to `int?`/`bool?` since no conversion is needed for either. Dates are a plain `yyyy-MM-dd` text box, not a `DatePicker` (whose `Date` is a `DateTimeOffset`, not a `DateTime`), for the same reason. Saves through `<Table>Repo.AddAsync`/`UpdateAsync`; a lookup drop-down's options come straight from the target's `DbContext` (`context.Set<Parent>()`), not that parent's own repository, so the template never has to assume every foreign-keyed table also has a `CS_Repo` run against it.
  - **`WinUI3_DetailMasterScreen.tt`** (`RequiresChildTables=true`, `NeedsReferencedDisplayColumns=true`) — `WinUI3_DetailScreen.tt`'s exact form plus one read-only grid per table in `TableModel.ChildForeignKeys` (§6), only offered when there's at least one (a table with none is exactly what `WinUI3_DetailScreen.tt` is for). Per the user's own answer when asked how to handle several child tables ("just have the generated code assume all are grids and the developer can cut out what grids are not needed"), **every** child table gets a grid unconditionally — no project-setting exclusion list. `TableModel` is built for exactly one table per generation run, so the template never sees a CHILD table's own column list the way it sees the current table's; each grid's columns are instead discovered at **run time** via EF Core's own entity metadata (`_context.Model.FindEntityType(typeof(Child)).GetProperties()`, which excludes navigation properties for free) and read by plain reflection — column order and headers come from EF, not a second schema-introspection pass. A child grid only loads once the parent row exists (a brand-new, unsaved row has no child rows yet). A child whose foreign key back to this table spans more than one column, or doesn't reference an `int` column, is skipped.
  - Live-tested against a real duplicate-FK-per-column table, a second production database's `dbo.Products` table (`DOIProductTypeID`/`RDProductTypeID` each carry two separate FK constraints to the same parent, `ProductTypes`) — this surfaced and fixed a crash (`Dictionary.ToDictionary` on a duplicate key) in all three templates' column-lookup logic, present in `TS_Component.tt` too (confirmed live, not yet fixed there — tracked separately). `WinUI3_DetailMasterScreen.tt` was also verified against `Products`' 12 real child tables (some referencing it as `ProductID`, one as `OGASProductID`) with no name/property collisions. Like `WinUI3_JunctionEditor.tt`, none of the three were compiled in a live WinUI3 project.
...
- Additional SP templates: Insert, Delete (with `spCanDelete` check wired in).
